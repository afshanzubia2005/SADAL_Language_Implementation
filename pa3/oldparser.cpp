...
bool IfStmt(istream &in, int &line)
{
    ...
    // end change
    if (TempsResults[X.GetLexeme()].GetType() != VBOOL && X.GetToken() != NOT)
    {
        LexItem Y = Parser::GetNextToken(in, line);

        if (Y.GetToken() == IDENT && TempsResults[Y.GetLexeme()].IsBool() && X.GetToken()){
            if (TempsResults[Y.GetLexeme()].IsBool() == FALSE){
if (X.GetToken() == NOT){
        ...
        if (Y.GetToken() != EQ && Y.GetToken() != NEQ && Y.GetToken() != GTHAN && Y.GetToken() != LTHAN && Y.GetToken() != LTE && Y.GetToken() != GTE && Y.GetToken() != IDENT)
        {
            // cout << Y.GetToken() << endl;//remove
            ParseError(line, "Invalid expression type for an If condition");
            Parser::PushBackToken(Y);
            return false;
        }

        else
        {
            Parser::PushBackToken(X);
        }
    ...
    else if (X.GetToken() == NOT)
    {
        LexItem Y = Parser::GetNextToken(in, line);
        if (Y.GetToken() == IDENT && TempsResults[Y.GetLexeme()].IsBool())
        {
            if (TempsResults[Y.GetLexeme()].GetBool() == true)
            {
                LexItem word = Parser::GetNextToken(in, line);
                while (word.GetToken() != ELSIF)
                {
                    LexItem word = Parser::GetNextToken(in, line);
                }
            }
    ...
    else if (TempsResults[X.GetLexeme()].GetBool() == false)
    {
        ...
        word = Parser::GetNextToken(in, line); // word after elsif
        // check if the next condiiton is valid
        if (word.GetToken() != EQ && word.GetToken() != NEQ && word.GetToken() != GTHAN && word.GetToken() != LTHAN && word.GetToken() != LTE && word.GetToken() != GTE)
        {
            ParseError(line, "Invalid expression type for an If condition");
...
bool AssignStmt(istream &in, int &line)
{
    ...
    {
        value1.SetType(VINT);
        value1.SetInt(stoi(lex1.GetLexeme()));
    }

    else if (lex1 == FCONST)
    {
        value1.SetType(VREAL);
        value1.SetReal(stof(lex1.GetLexeme()));
    }
        ...
        if (t == ASSOP)
        {
            ...
            if (!(value1.IsErr()) && !(value2.IsErr()))
            {
                ...
                if (value1.GetType() != value2.GetType())
                {
                    ...
                    // cout << value1.GetType() << endl; //debug
...
bool SimpleExpr(istream &in, int &line, Value &retVal)
{
    ...
    /*
        ...
        Token current_token = lex1.GetToken();
        Parser::PushBackToken(lex1); */
    ...
    while (tok == PLUS || tok == MINUS || tok == CONCAT)
    {
        ...
        {
            d.i = retVal.GetInt(); // initial value
        }
        else if (retVal.GetType() == VREAL)
        {
            d.f = retVal.GetReal(); // initial value
        }
        ...
        {
            d.i = d.i + retVal.GetInt(); // initial value
            retVal.SetInt(d.i);
        }
        else if (retVal.GetType() == VREAL)
        {
            d.f = d.f + retVal.GetReal(); // initial value
            retVal.SetReal(d.i);
        }
...
bool SimpleExpr(istream &in, int &line, Value &retVal)
{
    ...
    LexItem lex1 = Parser::GetNextToken(in, line);
    // cout << lex1.GetLexeme() << endl; // remove
    retVal = TempsResults[lex1.GetLexeme()];
    Parser::PushBackToken(lex1);
    ...
    if (tok.GetToken() == EQ)
    {
        LexItem nextword = Parser::GetNextToken(in, line);
        if (TempsResults[lex1.GetLexeme()].GetType() == VINT && TempsResults[lex1.GetLexeme()].GetInt() != stoi(nextword.GetLexeme()))
        {
            ...
            // Parser::PushBackToken(nextword);
            cout << "here" << endl; // remove
            // cout << nextword.GetLexeme() << endl;//remove // its 0
            return true;
        }
        nextword = Parser::GetNextToken(in, line);
        return true;
    }
    ...
    if (tok == PLUS || tok == MINUS || tok == CONCAT)
    {
...

        {
            ...
            // remove --> check the type of next_lex

            if (TempsResults[next_lex.GetLexeme()].GetType() == VINT && retVal.GetType() == VINT)
            {
                d.i = retVal.GetInt() + TempsResults[next_lex.GetLexeme()].GetInt();
                retVal.SetInt(d.i);
            }
            else if (TempsResults[next_lex.GetLexeme()].GetType() == VREAL && retVal.GetType() == VREAL)
            {
        ...
        else if (tok == CONCAT)
        {
            ...
            if ( ( retVal.IsString() || retVal.IsChar()) && (TempsResults[next_lex.GetLexeme()].IsString() || TempsResults[next_lex.GetLexeme()].IsChar())){
    ...
    // cout << tok << ", " << tok.GetLexeme() << endl;//remove
    //  its w2 here
    Parser::PushBackToken(tok);
    return true;
} // End of SimpleExpr
...
bool Term(istream &in, int &line, int sign, Value &retVal)
{
    ...
    // retVal now has the correct type --> VREAL ideally
    // cout << "testing out retVal" << retVal.GetType() <<endl;// test it out - should give 1
    LexItem tok;
    if (!t1)
    {
        return false;
    ...
    while (tok == MULT || tok == DIV || tok == MOD)
    {
        ...
        // cout << "second token: " << tok2.GetToken() << "," << tok2.GetLexeme() << endl; // remove

        if ((tok2.GetToken() == FCONST && retVal.GetType() == VREAL))
        {
            if (tok == MULT)
            {
                d.f = stof(tok2.GetLexeme()) * retVal.GetReal();
                retVal.SetReal(d.f);
                return true;
            }
        }

        if (tok2.GetToken() == ICONST && retVal.GetType() == VINT)
        {
            if (tok == MULT)
            {
                d.i = stoi(tok2.GetLexeme()) * retVal.GetInt();
                retVal.SetInt(d.i);
                return true;
            }
        }

        if (TempsResults[tok2.GetLexeme()].GetType() == retVal.GetType())
        {
            ...
            if (retVal.GetType() == VINT)
            {
                if (tok == MULT)
                {
                    d.i = retVal.GetInt() * TempsResults[tok2.GetLexeme()].GetInt();
                }
                else if (tok == DIV)
                {
                    d.i = retVal.GetInt() / TempsResults[tok2.GetLexeme()].GetInt();
            ...
            else if (retVal.GetType() == VREAL)
            {
                ...
                if (tok == MULT)
                {
                    d.i = retVal.GetReal() * TempsResults[tok2.GetLexeme()].GetReal();
                }
                else if (tok == DIV)
                {
                    d.i = retVal.GetReal() / TempsResults[tok2.GetLexeme()].GetReal();
        ...
        {
            if (first_token != second_token && second_token != SEMICOL)
            {
...
bool Factor(istream &in, int &line, int sign, Value &retVal)
{
    ...
    // cout << "first" << tok.GetToken() << "," << tok.GetLexeme() << endl; // remove

    if (first_token == IDENT)
    {
        if (TempsResults[tok.GetLexeme()].GetType() == VREAL)
        {
            // it will now be float num
            retVal.SetType(VREAL);
        }
    }
    ...
    if (tok2.GetToken() == EXP)
    {
        ...
        else if (TempsResults[tok.GetLexeme()].IsReal() && tok3.GetToken() == FCONST)
        {
            d.f = pow((TempsResults[tok.GetLexeme()].GetReal()), stof(tok3.GetLexeme()));
            retVal.SetType(VREAL);
            retVal.SetReal(d.f);
            LexItem tok4 = Parser::GetNextToken(in, line);
            if (tok4 == SEMICOL)
